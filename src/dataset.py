...
def load_RenderedSST2(transform = rgb_transform):
    dataset = datasets.RenderedSST2("data", download=True, transform=transform)
    return dataset, "RenderedSST2", len(dataset.classes)
...
def load_StanfordCars(transform = rgb_transform):
    dataset = datasets.StanfordCars("data", download=True, transform=transform)
    return dataset, "StanfordCars", len(dataset.classes)

#3
def load_STL10(transform = rgb_transform):
    dataset = datasets.STL10("data", download=True, transform=transform)
    return dataset, "STL10", 10

#3
def load_USPS(transform = grayscale_transform):
    dataset = datasets.USPS("data", download=True, transform=transform)
...
DATASET_GROUP_2 = [load_FashionMNIST, load_FER2013, load_FGVCAircraft, load_Flowers102, load_Food101, load_GTSRB]

DATASET_GROUP_3 = [load_USPS, load_STL10, load_StanfordCars, load_SEMEION, load_RenderedSST2]
