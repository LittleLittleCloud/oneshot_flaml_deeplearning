    ...
    for dataset_loader in dataset:
        try:
            ...
            if os.path.exists(f'{validate_json_folder}/{name}.json'):
                ...
                continue
            res = search_config(dataset, name, num_class=num_class, subsample=-1, device=device, trail_attempt=-1, seed=SEED, config_json_folder=validate_json_folder, epoch=20)
            print(res)
